...
export default function ProcessScheduler() {
  ...
  const [rutasArchivosSubidos, setRutasArchivosSubidos] = useState([]); 
  ...
  const [procesamientoCsvTerminado, setProcesamientoCsvTerminado] =
    useState(false);
  ...
  const sendMessage = (messageObject) => {
    if (ws.current && ws.current.readyState === WebSocket.OPEN) {
      ...
      setServerResponseMessage(
        "Error: No se pudo enviar mensaje. WebSocket desconectado."
      );
    }
  };
      ...
      try {
        ...
        switch (message.tipo) {
          ...
          case "evento_disparado":
            if (subscribedEvents.has(message.evento)) {
              ...
              setServerResponseMessage(
                `Evento '${
                  message.evento
                }' recibido! Solicitando procesamiento CSV ${
                  numArchivos > 0
                    ? `para ${numArchivos} archivo(s) seleccionado(s)`
                    : "(se procesarán todos los archivos por defecto en el servidor)"
                }...`
              );
              ...
              sendMessage({
                ...
                rutas_archivos_subidos: rutasArchivosSubidos,
              });
            }
            break;
          ...
          case "confirmacion_config_threads":
            setServerResponseMessage(
              `Configuración de concurrencia (${message.threads} ${message.concurrency_mode}(s)) confirmada.`
            );
            break;
          case "csv_actualizacion_fila":
            if (message.fila_csv && typeof message.fila_csv === "object") {
              setCsvFilasEnProgreso((prevFilas) => [
                ...prevFilas,
                message.fila_csv,
              ]);
              setShowCsvTable(true);
          ...
          case "procesamiento_csv_terminado":
            setServerResponseMessage(
              `Procesamiento CSV en servidor terminado. Estado: ${
                message.data?.status || "finalizado"
              }.`
            );
            setProcesamientoCsvTerminado(true);
            setShowCsvTable(true);
            if (
              message.data?.status === "fallido_script" ||
              message.data?.status === "completado_inesperado"
            ) {
              console.error(
                "DEBUG: Fallo en script de servidor:",
                message.data
              );
            }
            break;
        ...
        console.error(
          "DEBUG: Error procesando mensaje WS:",
          error,
          "Raw data:",
          event.data
        );
  ...
  useEffect(() => {
    ...
    wsInstance.onopen = () => {
      setIsConnected(true);
      setServerResponseMessage("Conectado al Servidor WebSocket.");
      sendMessage({ tipo: "listar_eventos" });
    };
    wsInstance.onclose = (event) => {
      ...
      if (event.wasClean) {
        setServerResponseMessage(
          `Desconectado del Servidor WebSocket (código: ${event.code}).`
        );
      } else {
        setServerResponseMessage(
          `Conexión perdida con el Servidor WebSocket (código: ${event.code}). Verifique que el servidor esté corriendo.`
        );
      }
    };
    wsInstance.onerror = (error) => {
      ...
      setIsConnected(false);
      setServerResponseMessage(
        "Error en la conexión WebSocket. Asegúrate de que el servidor Python esté ejecutándose."
      );
    };
    return () => {
      if (ws.current) {
  ...
  const onFileChange = async (e) => {
    ...
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      console.log(
        `DEBUG: Procesando archivo seleccionado #${i + 1}: ${
          file.name
      ...
      if (!file.name.toLowerCase().endsWith(".txt")) {
        console.warn(
          `DEBUG: Archivo '${file.name}' omitido porque no es .txt.`
        );
      ...
      try {
        ...
        console.log(
          `DEBUG: Respuesta HTTP recibida para '${file.name}'. Status: ${response.status}`
        );
        const responseText = await response.text();
        console.log(
          `DEBUG: Texto de respuesta del servidor para '${file.name}':`,
          responseText
        );

        if (!response.ok) {
          console.error(
            `DEBUG: Error HTTP ${response.status} subiendo ${file.name}. Respuesta: ${responseText}`
          );
          throw new Error(
            `Error HTTP ${response.status} subiendo ${file.name}: ${
        ...
        console.log(
          `DEBUG: Datos JSON parseados de la respuesta para '${file.name}':`,
          data
        );
        ...
        if (
          ...
          console.log(
            `DEBUG: Archivo '${file.name}' subido exitosamente. Ruta: ${data.filePath}`
          );
        } else {
          console.error(
            `DEBUG: Respuesta de subida inválida para '${file.name}'. 'success' no es true o 'filePath' falta. Data:`,
            data
          );
          throw new Error(
            data.message ||
              `Respuesta de subida inválida del servidor HTTP para ${file.name}.`
          );
        }
      } catch (error) {
        console.error(
          `DEBUG: CATCH - Error durante la subida o procesamiento de respuesta para '${file.name}':`,
          error.message
        );
        setServerResponseMessage(
          `Error al subir ${file.name}: ${error.message}. Revisa la consola del navegador y del servidor Node.js.`
        );
      }
    }

    console.log(
      `DEBUG: Finalizado el bucle de subida. ${newUploadedFilePaths.length} archivos subidos exitosamente en esta tanda.`
    );
    ...
    if (newUploadedFilePaths.length > 0) {
      ...
      setServerResponseMessage(
        `Subida completada. ${newUploadedFilePaths.length} archivo(s) listos en servidor.`
      );
      console.log(
        "DEBUG: Rutas de archivos actualizadas en el estado:",
        newUploadedFilePaths
      );
    } else {
     
      console.warn(
        "DEBUG: newUploadedFilePaths está vacío después de intentar subir los archivos seleccionados."
      );
      if (rutasArchivosSubidos.length === 0) {
        
        setServerResponseMessage(
          "No se pudo subir ningún archivo .txt válido de la selección actual."
        );
      }
    }
  ...
  const handleRemoveFile = (filePathToRemove) => {
    ...
    setRutasArchivosSubidos((prevPaths) => {
      ...
      if (newPaths.length === 0) {
        setServerResponseMessage(
          "Todos los archivos seleccionados han sido quitados."
        );
      }
      return newPaths;
    });
  ...
  const handleSendServerConfig = () => {
    ...
    if (!optionType) {
      alert("Selecciona un modo de concurrencia (Threads o Processes).");
      return;
    }
    const count = parseInt(optionCount, 10);
    if (isNaN(count) || count < 1) {
      alert(
        `Ingresa un número válido de ${
          optionType === "thread" ? "Threads" : "Processes"
        }`
      );
      return;
    }
    sendMessage({
      tipo: "configurar_threads_cliente",
      concurrency_mode: optionType,
      threads: count,
    });
    setServerResponseMessage(
      `Configuración (${count} ${
  ...
  const simulate = () => {
    ...
    if (at.length !== bt.length || at.length === 0) {
      alert("Llegada/Ráfaga inválidos.");
      setResults({ solvedProcessesInfo: [], ganttChartInfo: [] });
      return;
    }
    ...
    if (["NPP", "PP"].includes(algorithm)) {
      pr = priorityInput
        ...
        .map(Number)
        .filter((n) => !isNaN(n));
      if (pr.length !== at.length) {
        alert("Prioridades no coinciden.");
        setResults({ solvedProcessesInfo: [], ganttChartInfo: [] });
        return;
      }
    }
    const tq = Number(timeQuantum);
    if (algorithm === "RR" && (isNaN(tq) || tq < 1)) {
  ...
  const descargarCsvStream = () => {
    ...
    try {
      ...
      if (encabezados.length === 0) {
        setServerResponseMessage("No se pudieron determinar los encabezados.");
        return;
      }
      ...
      csvFilasEnProgreso.forEach((fila) => {
        csvContenido +=
          encabezados.map((enc) => escapeCsvCell(fila[enc])).join(",") + "\n";
      });
      ...
      console.error("DEBUG: Error al descargar CSV:", error);
      setServerResponseMessage("Error al generar/descargar CSV.");
    }
  };
  ...
  const avgTAT = solvedProcessesInfo.length
    ...
    : 0;
  const avgWT = solvedProcessesInfo.length
  ...
  const GanttChart = () => {
    if (
      ...
      typeof ganttChartInfo[0].start === "undefined"
    ) {
      return null;
    }
    ...
    return (
      <Box sx={{ mt: 3 }}>
        <Typography variant="h6" gutterBottom>
          Diagrama de Gantt (Simulación Local)
        </Typography>
        <CardContent
          sx={{
            overflowX: "auto",
        ...
        >
          ...
          >
            {ganttChartInfo.map((seg, i) => {
              const colorIndex =
                typeof seg.jobId === "number"
                  ? seg.jobId % COLORS.length
                  : i % COLORS.length;
              return (
                <React.Fragment key={`seg-${i}`}>
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{
                      width: `${Math.max(0, seg.stop - seg.start) * scale}px`,
                    }}
                    transition={{ duration: 0.4, delay: i * 0.1 }}
                    style={{
                      position: "absolute",
                      left: `${Math.max(0, seg.start - offset) * scale}px`,
                      height: 36,
                  ...
                  {i < ganttChartInfo.length - 1 &&
                    ganttChartInfo[i + 1].start > seg.stop && (
                      <Box
                        sx={{
                          position: "absolute",
                          left: `${(seg.stop - offset) * scale}px`,
                          width: `${
                            (ganttChartInfo[i + 1].start - seg.stop) * scale
                          }px`,
                          height: 36,
                          bgcolor: "grey.300",
                          display: "flex",
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 3, fontFamily: "Roboto, sans-serif" }}>
      ...
      <Card sx={{ mb: 3, boxShadow: 3 }}>
        <CardHeader
          title="Simulación de Planificador (Local)"
          sx={{ bgcolor: "grey.200" }}
        />
        <CardContent>
          <Stack spacing={2.5}>
            <FormControl size="small" fullWidth>
              <InputLabel id="algo-label-local">
                Algoritmo de Planificación
              </InputLabel>
              <Select
                labelId="algo-label-local"
            ...
            <TextField
              label="Tiempos de Llegada (separados por espacio)"
            ...
            />
            <TextField
              label="Tiempos de Ráfaga (separados por espacio)"
            ...
            {["NPP", "PP"].includes(algorithm) && (
              <TextField
                label="Prioridades (algoritmos de Prioridad)"
                placeholder="Ej: 2 1 3 (menor es más prioritario)"
                fullWidth
                value={priorityInput}
          ...
          <CardContent>
                ...
                {solvedProcessesInfo.map((row) => (
                  <TableRow key={row.job} hover>
                    ...
                    <TableCell align="center">
                      {["NPP", "PP"].includes(algorithm) ? row.pr : "-"}
                    </TableCell>
            ...
            <Box sx={{ mt: 2, mb: 2 }}>
              <Typography variant="subtitle1">
                Promedio Turnaround Time: {avgTAT.toFixed(2)}
              </Typography>
              <Typography variant="subtitle1">
      ...
      <Card sx={{ mb: 3, boxShadow: 3 }}>
        <CardHeader
          title="Suscripción a Eventos del Servidor"
          sx={{ bgcolor: "grey.200" }}
        />
        <CardContent>
          <Typography variant="h6" gutterBottom>
          ...
          {isConnected && Object.keys(availableEvents).length > 0 ? (
            ...
            >
              {Object.entries(availableEvents).map(([evName, evDesc]) => (
                <ListItem
                  key={evName}
                  divider
                  secondaryAction={
                    subscribedEvents.has(evName) ? (
                      <Button
                        size="small"
                        variant="outlined"
                        color="error"
                        onClick={() => handleUnsubscribe(evName)}
                      ...
                      <Button
                        ...
                        variant="contained"
                        color="primary"
                        onClick={() => handleSubscribe(evName)}
                        disabled={!isConnected}
                      >
                        Suscribir
                      </Button>
                ...
                >
                  ...
                  <ListItemText
                    primary={evName}
                    secondary={evDesc || "Evento del servidor"}
                  />{" "}
                </ListItem>
              ))}
            </List>
            ...
            <Typography variant="body2" color="text.secondary">
              {" "}
              {isConnected
                ? "No hay eventos definidos o actualizando..."
                : "Conéctate para ver eventos."}{" "}
            </Typography>
          )}
          <Typography variant="h6" sx={{ mt: 2.5 }} gutterBottom>
            Suscritos Actualmente:
          </Typography>
          {subscribedEvents.size > 0 ? (
            <List dense>
              {" "}
              {Array.from(subscribedEvents).map((ev) => (
                <ListItemText primary={`- ${ev}`} key={ev} />
              ))}{" "}
            </List>
          ) : (
            <Typography variant="body2" color="text.secondary">
      ...
      <Card sx={{ mb: 3, boxShadow: 3 }}>
        <CardHeader
          title="Archivo(s) TXT para Procesamiento en Servidor"
          sx={{ bgcolor: "grey.200" }}
        />
        <CardContent>
          ...
          {rutasArchivosSubidos.length === 0 && (
            <Typography variant="body1" sx={{ mt: 1 }}>
              Ningún archivo seleccionado. Si se dispara un evento, se
              procesarán todos los archivos por defecto en el servidor.
            </Typography>
          )}
          <Typography
            variant="caption"
          ...
          >
            * La subida se realiza a un servidor HTTP auxiliar. La selección de
            archivos define el lote a procesar. Si no hay archivos
            seleccionados, el servidor procesará un conjunto por defecto.
          </Typography>
        </CardContent>
      </Card>
      ...
      <Card sx={{ mb: 3, boxShadow: 3 }}>
        <CardHeader
          title="Configuración de Concurrencia (para Servidor WebSocket)"
          sx={{ bgcolor: "grey.200" }}
        />
        <CardContent>
          <FormControl
          ...
          >
            <FormLabel component="legend" sx={{ mb: 1, fontWeight: "medium" }}>
              Modo de Concurrencia en Servidor
            </FormLabel>
            <RadioGroup
              row
              value={optionType}
          ...
          <Button
            variant="contained"
            color="primary"
            onClick={handleSendServerConfig}
            fullWidth
            disabled={!isConnected}
            size="large"
          >
      ...
      {showCsvTable && (
        <Card sx={{ mb: 3, boxShadow: 3 }}>
          <CardHeader
            title="Datos CSV Generados por el Servidor (Stream)"
            ...
            {csvFilasEnProgreso.length > 0 ? (
              <>
                <Typography variant="caption" display="block" gutterBottom>
                  {" "}
                  {procesamientoCsvTerminado
                    ? `Procesamiento completado. Total filas: ${csvFilasEnProgreso.length}`
                    : `Recibiendo datos... Filas: ${csvFilasEnProgreso.length}`}{" "}
                </Typography>
                <TableContainer
                  component={Paper}
                  sx={{ maxHeight: 450, border: "1px solid #e0e0e0" }}
                >
                  <Table
                    size="small"
                    stickyHeader
                    aria-label="tabla csv generado"
                  >
                    <TableHead>
                      <TableRow
                        sx={{
                          "& th": { fontWeight: "bold", bgcolor: "grey.100" },
                        }}
                      >
                        {Object.keys(csvFilasEnProgreso[0]).map((key) => (
                          <TableCell
                            key={key}
                    ...
                    <TableBody>
                      {csvFilasEnProgreso.map((fila, index) => (
                        <TableRow
                          key={index}
                          hover
                          sx={{
                ...
                {procesamientoCsvTerminado && (
                  <Button
                    onClick={descargarCsvStream}
                    sx={{ mt: 2 }}
                    variant="contained"
                    color="success"
                    disabled={csvFilasEnProgreso.length === 0 || !isConnected}
                    size="large"
                  >
                    {" "}
              ...
              <Typography variant="body1" color="text.secondary">
                ...
                {isConnected
                  ? procesamientoCsvTerminado
                    ? "Procesamiento completado. No se generaron datos."
                    : "Esperando datos del servidor..."
                  : "Conéctate para ver datos."}{" "}
              </Typography>
            )}
            {!procesamientoCsvTerminado && csvFilasEnProgreso.length > 0 && (
              <Typography
                variant="caption"
                sx={{ display: "block", mt: 1, fontStyle: "italic" }}
              >
                {" "}
                Actualizando tabla en tiempo real...{" "}
              </Typography>
            )}
          </CardContent>
        </Card>
      ...
      {!showCsvTable && (
        <Card sx={{ mb: 3, boxShadow: 3 }}>
          <CardHeader
            title="Datos CSV Generados por el Servidor"
            sx={{ bgcolor: "grey.200" }}
          />
          <CardContent>
            {" "}
            <Typography variant="body1" color="text.secondary">
              {" "}
              {isConnected
                ? "Los datos procesados aparecerán aquí al activar un evento suscrito."
                : "Conéctate, sube archivos y suscríbete a un evento."}{" "}
            </Typography>{" "}
          </CardContent>
        </Card>
      )}
