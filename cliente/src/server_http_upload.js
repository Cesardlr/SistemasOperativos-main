...
// multer.diskStorage permite personalizar la ubicación y el nombre del archivo subido
...
// multer configura el almacenamiento y el filtro de archivos
const upload = multer({
  storage: storage,
...
app.post(
  ...
  (req, res) => {
    if (!req.file) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "No se subió ningún archivo o el tipo no es .txt",
        });
    }
  ...
  (error, req, res, next) => {
    ...
    if (error instanceof multer.MulterError) {
      return res
        .status(400)
        .json({ success: false, message: `Error de Multer: ${error.message}` });
    } else if (error) {
...
app.get("/", (req, res) => {
  res.send(
    "Servidor HTTP para subida de archivos está activo. Usa POST /upload-txt para subir."
  );
});

app.listen(port, () => {
  console.log(
    `Servidor HTTP para subida de archivos escuchando en http://localhost:${port}`
  );
  console.log(
    `Los archivos se guardarán en: ${path.resolve(UPLOAD_DIR)} (ruta absoluta)`
  );
});
